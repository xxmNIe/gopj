...
type StreamServiceClient interface {
	...
	Router(ctx context.Context, opts ...grpc.CallOption) (StreamService_RouterClient, error)
}
...
func (c *streamServiceClient) Router(ctx context.Context, opts ...grpc.CallOption) (StreamService_RouterClient, error) {
	stream, err := c.cc.NewStream(ctx, &_StreamService_serviceDesc.Streams[2], "/proto.StreamService/Router", opts...)
	if err != nil {
		return nil, err
	}
	x := &streamServiceRouterClient{stream}
	return x, nil
}

type StreamService_RouterClient interface {
	Send(*StreamRequest) error
	Recv() (*StreamResponse, error)
	grpc.ClientStream
}

type streamServiceRouterClient struct {
	grpc.ClientStream
}

func (x *streamServiceRouterClient) Send(m *StreamRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *streamServiceRouterClient) Recv() (*StreamResponse, error) {
	m := new(StreamResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type StreamServiceServer interface {
	...
	Router(StreamService_RouterServer) error
...
func (UnimplementedStreamServiceServer) Router(StreamService_RouterServer) error {
...
func _StreamService_Router_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(StreamServiceServer).Router(&streamServiceRouterServer{stream})
}

type StreamService_RouterServer interface {
	Send(*StreamResponse) error
	Recv() (*StreamRequest, error)
	grpc.ServerStream
}

type streamServiceRouterServer struct {
	grpc.ServerStream
}

func (x *streamServiceRouterServer) Send(m *StreamResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *streamServiceRouterServer) Recv() (*StreamRequest, error) {
	m := new(StreamRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var _StreamService_serviceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "Router",
			Handler:       _StreamService_Router_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
